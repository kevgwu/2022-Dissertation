''' Model 4 trains a 2-branch CNN using the original and context tiles'''
...
X_trainc=tf.convert_to_tensor(X_trainc)
X_valc=tf.convert_to_tensor(X_valc)
X_testc=tf.convert_to_tensor(X_testc)
...
#Defining early EarlyStopping
es=[EarlyStopping(monitor='val_loss',mode='min',verbose=1,patience=10, restore_best_weights=True)]

##---First Run
#Freezing all layers but last FC layers
for layer in cnn_model.layers[0:-4]:
    layer.trainable=False

#Train last layers
cnn_model.compile(optimizer=opt1, loss='mean_absolute_error', metrics=['mean_absolute_error'])
cnn_model.fit(x=[X_train,X_trainc],y=Y_train, batch_size=32, epochs=50, verbose=1, validation_data=([X_val,X_valc],Y_val),callbacks=[es])
...
history=cnn_model.fit(x=[X_train,X_trainc],y=Y_train, batch_size=32, epochs=50, verbose=1, validation_data=([X_val,X_valc],Y_val),callbacks=[es])

#Predicting Test Data & Saving to CSV
start_time = time.time()
Y_pred=cnn_model.predict([X_test,X_testc])
end_time = time.time()
...
final_df.to_csv('results/model4/model4_df{}.csv'.format(j),index=False)
